...
$.currentWindow = function () {
    ...
    var curTabWin = null;
    var curTab = parent.$('#mainTab').tabs('getSelected');
...
$.modalPrompt = function (options, callBack) {
    ...
    top.layer.prompt({
        ...
        function (text, index) {
            ...
            callBack(true, text);
            //top.layer.msg('演示完毕,您最后写下了：' + text);
        }
    );
    //top.layer.prompt({ title: '随便写点啥，并确认', formType: 2 }, function (text, index) {
    //    top.layer.close(index);
...
$.modalAlertFun = function (content, callBack) {
    top.layer.alert(content, {
        icon: "fa-exclamation-circle",
        title: "系统提示",
        btn: ['确认'],
...
$.submitFormByTab = function (options) {
    var defaults = {
        url: "",
        param: [],
        loading: "正在提交数据...",
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.state == "success") {
                ...
                if (options.close == true) {
                    var currtab_title = top.$('.tabs-selected .tabs-inner span').text();
                    top.$('#mainTab').tabs('close', currtab_title);
                }
            } else {
                $.modalAlert(data.message, data.state);
            }
...
$.submitFormToRefresh = function (options) {
    var defaults = {
        url: "",
        param: [],
        loading: "正在提交数据...",
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.state == "success") {
                ...
                if (options.close == true) {
                    var url = top.$(".tabs-panels .panel").eq($('.tabs-selected').index()).find("iframe").attr("src");
                    top.$(".tabs-panels .panel").eq($('.tabs-selected').index()).find("iframe").attr("src", url);
                }
            } else {
                $.modalAlert(data.message, data.state);
            }
...
//$.statusUpdateForm = function (options) {
//    var defaults = {
//        prompt: "您确定要修改该项数据的状态吗？",
//        url: "",
//        param: [],
//        loading: "正在修改数据...",
//        success: null,
        ...
        $.ajax({
            ...
            success: function (data) {
                $.each(data, function (i) {
                    var dis = data[i][options.disabled];
                    if (dis != null && dis != "") {
                        $element.append($("<option disabled='disabled'></option>").val(data[i][options.id]).html(data[i][options.text]));
                    }
                    else {
